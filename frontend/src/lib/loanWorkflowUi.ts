...
function formatRoleLabel(role: string): string {
  ...
  if (r === 'super_admin' || r === 'admin' || r === 'sub_admin') return 'Admin Approval';
...
/** Friendly actor role for timeline (who actually clicked approve/reject). */
export function formatLoanActorRoleLabel(role?: string | null): string {
  const r = String(role || '').trim();
  if (!r) return '';
  if (r === 'super_admin' || r === 'admin' || r === 'sub_admin') return 'Admin';
  if (r === 'hr') return 'HR';
  if (r === 'hod') return 'HOD';
  if (r === 'manager') return 'Manager';
  if (r === 'final_authority') return 'Final Authority';
...
/**
 * Hide redundant final_authority row when configured steps already cover the flow (leave style).
 */
export function getLoanDisplayApprovalChain(
  ...
  const steps = (loanSettings?.workflow?.steps || []).filter((s) => s.isActive !== false);
  const finalRole = resolveFinalAuthRole(loanSettings);

  // Prefer configured desks only — drop synthetic Final Approval when steps exist
  // (unless final authority is a specific user and no matching desk).
  if (steps.length > 0 && finalRole !== 'specific_user') {
    const withoutFinal = chain.filter((s) => s.role !== 'final_authority');
    if (withoutFinal.length > 0) return withoutFinal;
  }

  if (!finalRole || finalRole === 'specific_user') return chain;
  const normalizedFinal =
    finalRole === 'super_admin' || finalRole === 'admin' ? 'hr' : finalRole;
  const hasMatchingRoleStep = chain.some(
    (s) => (s.role === finalRole || s.role === normalizedFinal) && s.role !== 'final_authority'
  );
...
function chainStepToTimelineStep(
  ...
  return {
    ...
    actionByRole: formatLoanActorRoleLabel(step.actionByRole) || step.actionByRole,
...
export function isLoanFinalApprovalStep(
  ...
  // Leave-style: last configured step (nextStepOnApprove null) finishes the request.
  const steps = (loanSettings?.workflow?.steps || []).filter((s) => s.isActive !== false);
  const currentConfigured = steps.find((s) => {
    const role = String(s.approverRole || '').trim();
    const normalized = role === 'super_admin' || role === 'admin' ? 'hr' : role;
    return normalized === next || role === next;
  });
  if (currentConfigured && currentConfigured.nextStepOnApprove == null) {
    return true;
  }

  // When final authority is mapped to a workflow role (e.g. HR / Admin→hr), that step is terminal.
  if (finalAuth?.role && finalAuth.role !== 'specific_user') {
    const normalizedFinal =
      finalAuth.role === 'super_admin' || finalAuth.role === 'admin' ? 'hr' : finalAuth.role;
