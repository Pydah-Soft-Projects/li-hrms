    ...
    const handleLinkDepartments = async (e: React.FormEvent, force = false) => {
        ...
        try {
            ...
            if (res.success) {
                ...
                // Backend found employees in departments being unlinked — ask user
                // Safe departments + adds were already applied by backend
                setLinkConfirmData({
                ...
                {showLinkDeptDialog && (
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                        ...
                        <div className="relative z-50 w-full max-w-lg rounded-3xl border border-slate-200 bg-white p-6 shadow-2xl dark:border-slate-800 dark:bg-slate-950">
                            ...
                            {/* ── Confirmation overlay when departments have employees ── */}
                            {linkConfirmData ? (
                                <div className="space-y-4">
                                    ...
                                    <div className="space-y-2 pt-2">
                                        <button
                                            ...
                                            className="w-full rounded-2xl bg-amber-500 px-4 py-3 text-sm font-semibold text-white hover:bg-amber-600 shadow-lg shadow-amber-500/30"
                                        >
                                            Yes, Unlink Anyway
                                        </button>
                                        <button
                                            type="button"
                                            onClick={() => { setLinkConfirmData(null); setShowLinkDeptDialog(null); setLinkDeptSearch(''); loadData(); }}
                                            className="w-full rounded-2xl bg-emerald-500 px-4 py-3 text-sm font-semibold text-white hover:bg-emerald-600 shadow-lg shadow-emerald-500/30"
                                        >
                                            Keep Safe Changes Only
                                        </button>
                                        <button
                                            type="button"
                                            onClick={() => setLinkConfirmData(null)}
