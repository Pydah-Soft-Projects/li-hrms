...
export default function AttendancePage() {
  ...
  // Same logic as superadmin attendance / superadmin Leave–OD: prefer division.departments when present;
  // if that yields no rows in the loaded catalog, fall back to department ↔ division fields (incl. division_id).
  ...
  /** Zero-padded HH:MM for OT preview / confirm dialogs */
  ...
  /** Turn backend step tokens like 3.8900h into HH:MM for display */
      ...
      <div className="relative z-10 mx-auto max-w-[1920px] px-2 sm:px-4 md:px-6">
        ...
        {isEmployee && filteredMonthlyData.length > 0 && (() => {
          ...
          const totalLeaveDays =
            s?.totalLeaves ?? sumLeaveCreditFromDailyRecords(item.dailyAttendance);
                        ...
                        <div className="flex items-center gap-2">
                          {tableType === 'ot' && !otAutoCreateEnabled && (
                            <input
                              ...
                              checked={
                                filteredMonthlyData.length > 0 &&
                                filteredMonthlyData.every((item) => selectedOtEmployees[item.employee._id])
                              }
                              onChange={(e) => {
                                const checked = e.target.checked;
                                const newSelections = { ...selectedOtEmployees };
                                filteredMonthlyData.forEach((item) => {
                                  newSelections[item.employee._id] = checked;
                                });
                                setSelectedOtEmployees(newSelections);
                              }}
                            />
                          )}
                          <span>Employee</span>
                ...
                <div className="space-y-4">
                  ...
                  {/* Late In Display - Support Multi-Shift */}
                    ...
                    {!!attendanceDetail.permissionRequests?.length && (
                      <div className="col-span-2 rounded-xl bg-cyan-50/50 dark:bg-cyan-900/10 border border-cyan-100 dark:border-cyan-800/50 p-4">
                        <label className="text-[10px] font-black uppercase tracking-widest text-cyan-600/70 dark:text-cyan-400/70">Permission Details</label>
                          ...
                          {attendanceDetail.permissionRequests.map((p: any) => {
                            ...
                            const parseTimeToMinutes = (
                              ...
                              if (hhmmMatch) {
                                ...
                                if (h >= 1 && h <= 11) candidates.push((h + 12) * 60 + m);
                                if (h === 12) candidates.push(m);
                            ...
                            const timeLabel =
                              type === 'mid_shift'
                                ? `${formatTime(p.permissionStartTime)} - ${formatTime(p.permissionEndTime)}`
                                ...
                                {type !== 'mid_shift' && (
                                  <div className="mt-2 grid grid-cols-1 sm:grid-cols-2 gap-2">
                                    <div className="rounded-md bg-slate-100 dark:bg-slate-800/70 px-2 py-1.5">
                                      ...
                                      <p className="text-[11px] font-bold text-slate-900 dark:text-slate-100">{formatTime(type === 'late_in' ? shiftStartRaw : shiftEndRaw)}</p>
                                    </div>
                                    <div className="rounded-md bg-slate-100 dark:bg-slate-800/70 px-2 py-1.5">
                                ...
                                <div className="mt-2 flex flex-wrap gap-2">
                                  {outRequired && (
                                    <span className={`inline-flex items-center px-2 py-1 rounded-md text-[10px] font-black uppercase tracking-wider ${outVerified ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-300' : 'bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-300'}`}>
                                      OUT QR {outVerified ? `Verified ${formatTime(p.gateOutTime)}` : 'Pending'}
                                    </span>
                                  )}
                                  {inRequired && (
                                    <span className={`inline-flex items-center px-2 py-1 rounded-md text-[10px] font-black uppercase tracking-wider ${inVerified ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-300' : 'bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-300'}`}>
                                      IN QR {inVerified ? `Verified ${formatTime(p.gateInTime)}` : 'Pending'}
                                    </span>
                                  )}
                                </div>
                              </div>
                                  ...
                                  <td className="px-4 py-3 font-semibold text-orange-600">
                                    ...
                                    {record?.leaveInfo ? (
                                      <span className="ml-1 block text-[10px] font-normal text-slate-500">
                                        {formatAttendanceLeaveDayPortion(record.leaveInfo)}
                                      </span>
                                    ) : null}
                                  </td>
