...
const expandRequestToSegments = (item: {
  fromDate?: string;
  toDate?: string;
  isHalfDay?: boolean;
  ...
  const from = normalizeRequestDate(item?.fromDate || '');
  const to = normalizeRequestDate(item?.toDate || item?.fromDate || '');
  if (!from || !to) return [];
  return expandLeaveToDailySegments({
    fromDate: from,
    toDate: to,
    isHalfDay: item?.isHalfDay,
    halfDayType: (item?.halfDayType as 'first_half' | 'second_half') || null,
    fromIsHalfDay: item?.fromIsHalfDay,
    toIsHalfDay: item?.toIsHalfDay,
  });
};
...
const findActiveHalfDayConflict = (
  ...
  const reqFrom = normalizeRequestDate(requested.fromDate);
  const reqTo = normalizeRequestDate(requested.toDate || requested.fromDate);

  if (reqIsHours && reqFrom) {
    for (const item of existingRequests) {
      if (!ACTIVE_REQUEST_STATUSES.has(String(item?.status || ''))) continue;
      const existFrom = normalizeRequestDate(item?.fromDate || '');
      const existTo = normalizeRequestDate(item?.toDate || item?.fromDate || '');
      if (!existFrom || reqFrom < existFrom || reqFrom > existTo) continue;
      ...
      if (requested.requestLabel === 'leave' || !item.odType_extended) {
        if (!item.isHalfDay && requested.requestLabel === 'leave') {
          return `An active full-day ${requested.requestLabel.toUpperCase()} conflicts on ${reqFrom}.`;
        }
      }

      if (isFullExisting) {
        return `An active full-day OD conflicts on ${reqFrom} with hour-based OD.`;
      }
      if (
        isHoursExisting &&
        item.odStartTime &&
        ...
        return `An active hour-based OD (${item.odStartTime}–${item.odEndTime}) overlaps your window on ${reqFrom}.`;
      }
    }
    return null;
  }

  const requestedSegments = expandLeaveToDailySegments(requested);

  for (const item of existingRequests) {
    if (!ACTIVE_REQUEST_STATUSES.has(String(item?.status || ''))) continue;
    const existingSegments = expandRequestToSegments(item);
    for (const reqSeg of requestedSegments) {
      const existSeg = existingSegments.find((e) => e.dateStr === reqSeg.dateStr);
      if (!existSeg) continue;
      if (
        checkDayHalfCoverageConflict(
          { isHalfDay: reqSeg.isHalfDay, halfDayType: reqSeg.halfDayType },
          { isHalfDay: existSeg.isHalfDay, halfDayType: existSeg.halfDayType }
        )
      ) {
        const halfLabel = (half: string | null) =>
          half === 'second_half' ? 'second-half' : half === 'first_half' ? 'first-half' : 'full-day';
        if (!reqSeg.isHalfDay || !existSeg.isHalfDay) {
          return `An active ${requested.requestLabel.toUpperCase()} conflicts on ${reqSeg.dateStr} (you: ${halfLabel(reqSeg.isHalfDay ? reqSeg.halfDayType : null)}, existing: ${halfLabel(existSeg.isHalfDay ? existSeg.halfDayType : null)}).`;
        }
        return `An active ${halfLabel(existSeg.halfDayType)} ${requested.requestLabel.toUpperCase()} already exists on ${reqSeg.dateStr}.`;
      }
    }
  }
...
function LeavesPageContent() {
  ...
  const handleApply = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Check for conflicts
      if (approvedRecordsInfo && isSingleDayRequest && formData.odType_extended !== 'hours') {
      ...
      if (approvedRecordsInfo) {
        ...
        if (formData.isHalfDay && formData.odType_extended !== 'hours') {
          const approvedHalfDayType = approvedRecordsInfo.hasLeave
      ...
      if (
        ...
        const hoursOds = approvedRecordsInfo.hoursOdsOnDate || [];
      ...
      if (formData.fromDate && employeeToApplyFor) {
        ...
        const requestedBoundary =
          applyType === 'leave'
            ? leaveBoundaryFromForm(formData)
            : {
        ...
        try {
          ...
          const conflictMessage = findActiveHalfDayConflict(
            ...
            { ...requestedBoundary, requestLabel: applyType }
          );
      ...
      if (applyType === 'leave') {
        ...
        payload.isHalfDay = norm.isHalfDay;
        payload.halfDayType = norm.halfDayType;
        payload.fromIsHalfDay = norm.fromIsHalfDay;
        payload.fromHalfDayType = norm.fromHalfDayType;
        payload.toIsHalfDay = norm.toIsHalfDay;
        payload.toHalfDayType = norm.toHalfDayType;
    ...
    try {
      ...
      if (response.success) {
        ...
        const classMsg =
          ...
          (response as any)?.data?.durationClassification?.employeeMessage;
        ...
        if (needsAuth && classMsg) {
          ...
          toast.success(
            typeLabel
              ? `OD OUT submitted as ${typeLabel}. Request moved to pending.`
                ...
                {applyType === 'od' && (
                  <div className="space-y-3">
                    ...
                    <LocationPhotoCapture
                      ...
                      allowDeviceFileUpload={canOdUploadFromDevice(currentUser)}
                      onCapture={(loc, photo) => {
                          ...
                          {['manager', 'hod', 'hr', 'super_admin', 'sub_admin'].includes(currentUser?.role || '') && !['rejected', 'cancelled'].includes(selectedItem!.status) && (
                            <button
                              ...
                              onClick={() => {
                                ...
                                const initialType = item?.odType_extended || (item?.isHalfDay ? 'half_day' : 'full_day');
                                setEditFormData({
                                  ...item,
                                  odType_extended: initialType,
                                  isHalfDay: initialType === 'half_day',
              ...
              <div className={`min-h-0 flex-1 overflow-y-auto overscroll-y-contain ${detailType === 'od' ? 'p-4 sm:p-5 md:p-6 space-y-4' : 'p-4 sm:p-6 md:p-8 space-y-6 sm:space-y-8'}`}>
                ...
                <div className="shrink-0 border-t border-slate-200 bg-slate-50 p-4 pb-[max(1rem,env(safe-area-inset-bottom,0px))] dark:border-slate-700 dark:bg-slate-900 flex flex-col gap-3">
                  ...
                  {detailType === 'od' && (selectedItem as any)?.durationClassification?.requiresAuthorityDecision &&
                    ...
                    canPerformAction(selectedItem, detailType) && (
...

      {
        showOutEvidenceDialog && selectedItem && detailType === 'od' && (
          <div className="fixed inset-0 z-[110] flex items-center justify-center overflow-y-auto overscroll-contain px-4 py-6 sm:p-4">
            ...
            <div className="relative z-[111] my-auto flex h-auto min-h-0 w-full max-w-lg max-h-[min(92dvh,calc(100dvh-3rem))] flex-col overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-2xl dark:border-slate-700 dark:bg-slate-900">
...

      {
        showEditDialog && selectedItem && (
          <div className="fixed inset-0 z-[120] flex items-center justify-center p-4">
            ...
            <div className="relative z-[121] w-full max-w-lg max-h-[90vh] overflow-y-auto rounded-3xl bg-white/90 dark:bg-slate-900/90 backdrop-blur-xl border border-white/20 dark:border-slate-800 shadow-2xl p-6 sm:p-8 animate-in zoom-in-95 duration-300">
              <div className="mb-8">
