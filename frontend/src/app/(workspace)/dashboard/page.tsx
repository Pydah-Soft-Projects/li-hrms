                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-sm md:text-lg font-black text-white font-mono">
                      {attendanceData?.[0]?.inTime
                        ? new Date(attendanceData[0].inTime).toLocaleTimeString('en-US', { timeZone: 'UTC', hour: '2-digit', minute: '2-digit' })
                        : (attendanceData?.[0]?.shifts?.[0]?.inTime
                          ? new Date(attendanceData[0].shifts[0].inTime).toLocaleTimeString('en-US', { timeZone: 'UTC', hour: '2-digit', minute: '2-digit' })
                          : '--:--')
                      }
                    </span>
                  </div>
                ...
                <div className="flex-1 md:flex-initial px-3 py-2 md:px-5 md:py-2.5 rounded-xl md:rounded-2xl bg-black/10 backdrop-blur-md border border-white/10 flex flex-col items-center min-w-[80px] md:min-w-[100px]">
                  ...
                  <span className="text-sm md:text-lg font-black text-white font-mono">
                    {attendanceData?.[0]?.outTime
                      ? new Date(attendanceData[0].outTime).toLocaleTimeString('en-US', { timeZone: 'UTC', hour: '2-digit', minute: '2-digit' })
                      : (attendanceData?.[0]?.shifts?.[0]?.shiftEndTime
                        ? new Date(`${new Date().toISOString().split('T')[0]}T${attendanceData[0].shifts[0].shiftEndTime}`).toLocaleTimeString('en-US', { timeZone: 'UTC', hour: '2-digit', minute: '2-digit' })
                        : '--:--')
                    }
                  </span>
                </div>
                    ...
                    {record.shifts && record.shifts.length > 0 ? (
                        ...
                        <div key={`${recordIdx}-${shiftIdx}`} className={`flex flex-col px-5 md:flex-row md:items-center md:justify-between gap-2 md:gap-0 ${shiftIdx > 0 ? 'mt-3 md:mt-4 pt-3 md:pt-4 border-t border-white/10' : ''}`}>
                          ...
                          <div className="flex gap-4 md:gap-8 w-full md:w-auto">
                            ...
                            <div className="flex flex-col flex-1 md:flex-initial md:items-center">
                              ...
                              <span className="text-xs md:text-sm font-bold text-white font-mono">{shift.inTime ? new Date(shift.inTime).toLocaleTimeString('en-US', { timeZone: 'UTC', hour: '2-digit', minute: '2-digit' }) : '--:--'}</span>
                            </div>
                            <div className="flex flex-col flex-1 items-end md:flex-initial md:items-end">
                              ...
                              <span className="text-xs md:text-sm font-bold text-white font-mono">{shift.outTime ? new Date(shift.outTime).toLocaleTimeString('en-US', { timeZone: 'UTC', hour: '2-digit', minute: '2-digit' }) : '--:--'}</span>
                            </div>
                          </div>
                        </div>
                      ))
                      ...
                      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-2 md:gap-0">
                        ...
                        <div className="flex gap-4 md:gap-8 w-full md:w-auto">
                          ...
                          <div className="flex flex-col flex-1 md:flex-initial md:items-center">
                            <span className="text-[8px] md:text-[10px] font-semibold text-emerald-100 uppercase tracking-wide md:tracking-wider">In Time</span>
                            <span className="text-xs md:text-sm font-bold text-white font-mono">{record.inTime ? new Date(record.inTime).toLocaleTimeString('en-US', { timeZone: 'UTC', hour: '2-digit', minute: '2-digit' }) : '--:--'}</span>
                          </div>
                          <div className="flex flex-col flex-1 items-end md:flex-initial md:items-end">
                            <span className="text-[8px] md:text-[10px] font-semibold text-emerald-100 uppercase tracking-wide md:tracking-wider">Out Time</span>
                            <span className="text-xs md:text-sm font-bold text-white font-mono">{record.outTime ? new Date(record.outTime).toLocaleTimeString('en-US', { timeZone: 'UTC', hour: '2-digit', minute: '2-digit' }) : '--:--'}</span>
                          </div>
                        </div>
                      </div>
                    )}
