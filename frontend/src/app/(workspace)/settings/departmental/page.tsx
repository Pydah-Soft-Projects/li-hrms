  ...
  return (
    <SettingsAccentShell className="flex min-h-screen items-start -m-4 sm:-m-5 lg:-m-6">
      ...
      >
        ...
        <nav className="settings-scrollbar flex-1 space-y-6 overflow-y-auto px-3 pb-6 sm:space-y-8 sm:px-4 sm:pb-8">
          {Object.entries(groupedDeptMenu).map(([group, items]) => (
            <div key={group} className="space-y-1">
              <h3
                className="mb-2 px-3 text-[10px] font-semibold uppercase tracking-[0.2em] sm:px-4"
                style={{ color: 'var(--ps-accent-ink)' }}
              >
                {group}
              </h3>
