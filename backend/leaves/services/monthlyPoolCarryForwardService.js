...
async function processPayrollCycleCarryForward(closingPayrollMonth, closingPayrollYear) {
  ...
  for await (const doc of cursor) {
    ...
    try {
      ...
      const clForfeit = clRoll ? 0 : roundHalf(alloc.clRem);
      const cclForfeit = cclRoll ? 0 : roundHalf(alloc.cclRem);
      const elForfeit = elRoll ? 0 : roundHalf(alloc.elRem);
      ...
      if (next && (clCarry > 0 || cclCarry > 0 || elCarry > 0)) {
        ...
        if (clCarry > 0) {
          await leaveRegisterService.addTransaction({
            ...
            endDate: anchor,
            days: clCarry,
            reason: `Carry forward — unused monthly apply pool CL from payroll ${fromLabel} → ${nextLabel} (after CL→CCL→EL priority vs ${alloc.U} day(s) toward cap).`,
            status: 'APPROVED',
            autoGenerated: true,
            autoGeneratedType: TX.CL_CARRY,
          });
        ...
        if (cclCarry > 0) {
          await leaveRegisterService.addTransaction({
            ...
            endDate: anchor,
            days: cclCarry,
            reason: `Carry forward — unused monthly apply pool CCL from payroll ${fromLabel} → ${nextLabel} (after CL→CCL→EL priority vs ${alloc.U} day(s) toward cap).`,
            status: 'APPROVED',
            autoGenerated: true,
            autoGeneratedType: TX.CCL_CARRY,
          });
          results.carriesPosted++;
        }
        if (elCarry > 0) {
          await leaveRegisterService.addTransaction({
            ...
            endDate: anchor,
            days: elCarry,
            reason: `Carry forward — unused monthly apply pool EL from payroll ${fromLabel} → ${nextLabel} (after CL→CCL→EL priority vs ${alloc.U} day(s) toward cap).`,
            status: 'APPROVED',
            autoGenerated: true,
            autoGeneratedType: TX.EL_CARRY,
          });
          results.carriesPosted++;
        }
