...
function sumSlotTxDays(slot, { leaveType, transactionType, autoGeneratedType }) {
    const wantLeaveType = String(leaveType || '').toUpperCase();
    const wantTxType = String(transactionType || '').toUpperCase();
    const wantAutoType = String(autoGeneratedType || '');
    let total = 0;
    for (const tx of slot?.transactions || []) {
        if (wantLeaveType && String(tx?.leaveType || '').toUpperCase() !== wantLeaveType) continue;
        if (wantTxType && String(tx?.transactionType || '').toUpperCase() !== wantTxType) continue;
        if (wantAutoType && String(tx?.autoGeneratedType || '') !== wantAutoType) continue;
        total += Math.max(0, Number(tx?.days) || 0);
    }
    return round2(total);
}

function reconcileMonthlyTransfersForLeaveChange(leaveRecord, options = {}) {
    try {
        const monthlyTransferReconciliationService = require('./monthlyTransferReconciliationService');
        monthlyTransferReconciliationService.scheduleTransferRebuildForLeaveChange(leaveRecord, options);
        return { ok: true, scheduled: true };
    } catch (error) {
        console.error('[LeaveRegister] Monthly transfer reconciliation schedule failed:', {
            leaveId: leaveRecord?._id ? String(leaveRecord._id) : null,
            employeeId: leaveRecord?.employeeId ? String(leaveRecord.employeeId) : null,
            error: error?.message || String(error),
        });
        return {
            ok: false,
            skipped: true,
            reason: 'monthly_transfer_reconciliation_failed',
            error: error?.message || String(error),
        };
    }
}
...
class LeaveRegisterService {
    ...
    async addLeaveDebit(leaveRecord, approvedByUserId = null) {
        try {
            ...
            const monthlyTransferReconciliation = await reconcileMonthlyTransfersForLeaveChange(leaveRecord);
            return {
                ...result,
    ...
    async addELUsedInPayroll(employeeId, days, month, payrollBatchId = null) {
        ...
        const transactionData = {
            ...
            leaveType: 'EL',
            transactionType: 'DEBIT',
            // Pin the payroll-use debit to the cycle END (same instant the EL accrual credit is posted),
            // not the cycle start. The balance recalc orders EL rows by startDate; if this debit were
            // dated at cycle start it would run BEFORE that cycle's EARNED_LEAVE credit, hit a smaller
            // opening balance, and get clamped to 0 (max(0, opening-days)) — swallowing the deduction so
            // the total looks like "previous + present". With equal startDate, transaction priority puts
            // the credit (EARNED_LEAVE = 20) before the DEBIT (50): credit first, then the deduction.
            startDate: periodInfo.payrollCycle.endDate,
            ...
            // Force the correct slot: startDate is now the cycle-end day, which can resolve to the next
            // cycle via getPeriodInfo; pin month/year so the debit lands in this payroll cycle's slot.
            payrollCycleMonth: periodInfo.payrollCycle.month,
            payrollCycleYear: periodInfo.payrollCycle.year,
        ...
        for (const entry of groupedData) {
            ...
            if (yd && Array.isArray(yd.months) && yd.months.length > 0) {
                const sumCl = yd.months.reduce((s, slot) => {
                    ...
                    const fallbackPolicy = Math.max(
                        0,
                        round2((Number(slot.clCredits) || 0) - round2(Number(pin.cl) || 0))
                    );
                    const scheduledTxCl = sumSlotTxDays(slot, {
                        leaveType: 'CL',
                        transactionType: 'CREDIT',
                        autoGeneratedType: 'MONTHLY_CL_SCHEDULE',
                    });
                    const policy = scheduledTxCl > 0 ? scheduledTxCl : fallbackPolicy;
                    return s + policy;
                }, 0);
                ...
                for (let orderedIdx = 0; orderedIdx < slotsOrdered.length; orderedIdx++) {
                    ...
                    monthsOut.push({
                        ...
                        scheduled: {
                            ...
                            monthlyClScheduledCredits: sumSlotTxDays(slot, {
                                leaveType: 'CL',
                                transactionType: 'CREDIT',
                                autoGeneratedType: 'MONTHLY_CL_SCHEDULE',
                            }),
            ...
            entry.registerMonths = monthsOut.map((m) => {
                ...
                const scheduledClFromTx = round2(Number(m.scheduled?.monthlyClScheduledCredits) || 0);
                const fallbackPolicyScheduledCl = m.scheduled ? Math.max(0, round2(fullClCr - clTransferIn)) : null;
                const policyScheduledCl =
                    m.scheduled && scheduledClFromTx > 0 ? scheduledClFromTx : fallbackPolicyScheduledCl;
                const effectiveScheduledCl =
                    m.scheduled && policyScheduledCl != null
                        ? Math.max(fullClCr, round2(policyScheduledCl + (clTransferIn || 0)))
                        : m.scheduled?.clCredits ?? null;
                ...
                const periodEndedForRegister =
                    !!m.payPeriodEnd &&
                    leaveRegisterYearService.isPayrollPeriodEndedOnOrBeforeAsOf(
                        m.payPeriodEnd,
                        asOfRegisterXfer
                    );
                ...
                const cclLockedDisplay = periodEndedForRegister
                    ? 0
                    : sub != null
                      ? Number(sub.pendingLockedCCL) || 0
                      : null;
                const elLockedDisplay = periodEndedForRegister
                    ? 0
                    : sub != null
                      ? Number(sub.pendingLockedEL) || 0
                      : null;
                ...
                const xferOutComputed = leaveRegisterYearService.computeRegisterMonthPoolTransferOutDisplay(
                    {
                        ...
                        lockedCl: clLockedDisplay,
                        lockedCcl: cclLockedDisplay ?? 0,
                        lockedEl: elLockedDisplay ?? 0,
                    },
                    asOfRegisterXfer,
                    policy
                );
                const pco = m.scheduled?.poolCarryForwardOut;
                const xferOut = {
                    cl:
                        periodEndedForRegister && pco?.cl != null
                            ? round2(Number(pco.cl) || 0)
                            : xferOutComputed.cl,
                    ccl:
                        periodEndedForRegister && pco?.ccl != null
                            ? round2(Number(pco.ccl) || 0)
                            : xferOutComputed.ccl,
                    el:
                        periodEndedForRegister && pco?.el != null
                            ? round2(Number(pco.el) || 0)
                            : xferOutComputed.el,
                };
                ...
                return {
                    ...
                    scheduledCl: effectiveScheduledCl,
                    policyScheduledCl,
                    scheduledClYtd,
