...
function ensureMonthlySubLedgerShape(sub) {
    ...
    if (sub.casualLeave.poolTransferInRaw == null) sub.casualLeave.poolTransferInRaw = 0;
    ...
    if (sub.earnedLeave.poolTransferInRaw == null) sub.earnedLeave.poolTransferInRaw = 0;
    ...
    if (sub.compensatoryOff.poolTransferInRaw == null) sub.compensatoryOff.poolTransferInRaw = 0;
    return sub;
}
...
/** Pool remainder for register/API / UI Bal: policy Cr + carried in − (used + lock) − transfer out. */
function registerPoolBalanceDisplay(policyCr, transferIn, used, locked, transferOut) {
    const crN = policyCr != null && Number.isFinite(Number(policyCr)) ? Number(policyCr) : null;
    ...
    const ul = u + l;
    const tout = transferOut != null && Number.isFinite(Number(transferOut)) ? Number(transferOut) : 0;
    if (crN === null && tin === null && used == null && locked == null) return null;
    return round2((crN ?? 0) + (tin ?? 0) - ul - tout);
}

/**
 * Sum CL/CCL/EL credits posted as monthly pool carry-in on the FY slot.
 * When `txs` is an array (including empty), returns the summed days (0 if no IN_* rows) so register xfer-in
 * matches the ledger and does not fall back to denormalized fields when posted amount is zero.
 * Returns null only when `txs` is missing (caller may fall back to poolCarryForwardIn).
 */
function sumMonthlyPoolTransferInFromFySlotTxs(txs, leaveType) {
    const want = String(leaveType || '').toUpperCase();
    if (!Array.isArray(txs)) return null;
    let sum = 0;
    for (const t of txs) {
        if (String(t.transactionType || '').toUpperCase() !== 'CREDIT') continue;
        if (String(t.leaveType || '').toUpperCase() !== want) continue;
        const ag = String(t.autoGeneratedType || '');
        if (!ag.startsWith('MONTHLY_POOL_TRANSFER_IN_')) continue;
        sum += round2(Number(t.days) || 0);
    }
    return round2(sum);
}

/**
 * Sum CL/CCL/EL debits posted as monthly pool carry-out on the FY slot.
 * Same contract as sumMonthlyPoolTransferInFromFySlotTxs: array => numeric sum (0 if no OUT_* rows).
 */
function sumMonthlyPoolTransferOutFromFySlotTxs(txs, leaveType) {
    const want = String(leaveType || '').toUpperCase();
    if (!Array.isArray(txs)) return null;
    let sum = 0;
    for (const t of txs) {
        if (String(t.transactionType || '').toUpperCase() !== 'DEBIT') continue;
        if (String(t.leaveType || '').toUpperCase() !== want) continue;
        const ag = String(t.autoGeneratedType || '');
        if (!ag.startsWith('MONTHLY_POOL_TRANSFER_OUT_')) continue;
        sum += round2(Number(t.days) || 0);
    }
    return round2(sum);
}
...
class LeaveRegisterService {
    ...
    async addEarnedLeaveCredit(employeeId, elEarned, month, year, calculationBreakdown = null, cycleEnd = null) {
        try {
            ...
            // Anchor on the payroll month (month/year) used for EL + MonthlyAttendanceSummary (YYYY-MM),
            // not only getPeriodInfo(cycleEnd) — avoids mis-posting to the wrong FY slot when cycleEnd
            // resolves differently from the intended pay period (e.g. slot lookup falling through).
            const m = Number(month);
            const y = Number(year);
            let cycleDate = cycleEnd;
            if (!cycleDate && Number.isFinite(m) && m >= 1 && m <= 12 && Number.isFinite(y) && y > 0) {
                const pc = await dateCycleService.getPayrollCycleForMonth(y, m);
                cycleDate = pc.endDate;
            }
            if (!cycleDate) {
                cycleDate = new Date(y, m - 1, 15);
            }
    ...
    async hydrateRegisterYearView(groupedData, financialYear, options = {}) {
        ...
        for (const entry of groupedData) {
            ...
            if (yd && Array.isArray(yd.months) && yd.months.length > 0) {
                ...
                for (let orderedIdx = 0; orderedIdx < slotsOrdered.length; orderedIdx++) {
                    ...
                    monthsOut.push({
                        ...
                        /** Pool carry-in from FY slot ledger (matches month transaction modal). */
                        slotPoolTransferIn: {
                            cl: sumMonthlyPoolTransferInFromFySlotTxs(slot.transactions, 'CL'),
                            ccl: sumMonthlyPoolTransferInFromFySlotTxs(slot.transactions, 'CCL'),
                            el: sumMonthlyPoolTransferInFromFySlotTxs(slot.transactions, 'EL'),
                        },
                        slotPoolTransferOut: {
                            cl: sumMonthlyPoolTransferOutFromFySlotTxs(slot.transactions, 'CL'),
                            ccl: sumMonthlyPoolTransferOutFromFySlotTxs(slot.transactions, 'CCL'),
                            el: sumMonthlyPoolTransferOutFromFySlotTxs(slot.transactions, 'EL'),
                        },
            ...
            entry.registerMonths = monthsOut.map((m) => {
                const pin =
                    m.scheduled &&
                    m.scheduled.poolCarryForwardIn &&
                    typeof m.scheduled.poolCarryForwardIn === 'object'
                        ? m.scheduled.poolCarryForwardIn
                        : {};
                const storedClTransferIn = m.scheduled != null ? round2(Number(pin.cl) || 0) : null;
                const storedCclTransferIn = m.scheduled != null ? round2(Number(pin.ccl) || 0) : null;
                const storedElTransferIn = m.scheduled != null ? round2(Number(pin.el) || 0) : null;
                ...
                const clCreditedNet = Math.max(
                    0,
                    clCredited - clReversal - (Number(clL.poolTransferInRaw) || 0)
                );
                ...
                /** Pool math: pipeline lock only (sub-ledger or stored monthly apply). Legacy slot `lockedCredits` ignored. */
                const storedApplyLock =
                    hasStored && st.locked != null && Number.isFinite(Number(st.locked))
                        ? Math.max(0, Number(st.locked))
                        : 0;
                const clLockForPool = sub != null ? clLockedDisplay : storedApplyLock;
                const cclLockForPool = sub != null ? Number(sub.pendingLockedCCL) || 0 : 0;
                const elLockForPool = sub != null ? Number(sub.pendingLockedEL) || 0 : 0;
                ...
                /** Prefer FY-slot MONTHLY_POOL_TRANSFER_IN_* totals (matches UI month modal); else denormalized poolCarryForwardIn. */
                const clTransferIn =
                    m.scheduled == null
                        ? null
                        : m.slotPoolTransferIn && m.slotPoolTransferIn.cl != null
                          ? m.slotPoolTransferIn.cl
                          : storedClTransferIn;
                const cclTransferIn =
                    m.scheduled == null
                        ? null
                        : m.slotPoolTransferIn && m.slotPoolTransferIn.ccl != null
                          ? m.slotPoolTransferIn.ccl
                          : storedCclTransferIn;
                const elTransferIn =
                    m.scheduled == null
                        ? null
                        : m.slotPoolTransferIn && m.slotPoolTransferIn.el != null
                          ? m.slotPoolTransferIn.el
                          : storedElTransferIn;
                const policyScheduledCl = m.scheduled ? Math.max(0, round2(fullClCr - (clTransferIn || 0))) : null;
                const policyScheduledCco = m.scheduled ? Math.max(0, round2(fullCco - (cclTransferIn || 0))) : null;
                const policyScheduledEl = m.scheduled ? Math.max(0, round2(fullElCr - (elTransferIn || 0))) : null;
                ...
                const xferScheduled =
                    m.scheduled == null
                        ...
                        : {
                              ...m.scheduled,
                              clCredits: round2(
                                  (Number(policyScheduledCl) || 0) + (Number(clTransferIn) || 0)
                              ),
                              compensatoryOffs: round2(
                                  (Number(policyScheduledCco) || 0) + (Number(cclTransferIn) || 0)
                              ),
                              elCredits: round2(
                                  (Number(policyScheduledEl) || 0) + (Number(elTransferIn) || 0)
                              ),
                          };

                const xferOutComputed = leaveRegisterYearService.computeRegisterMonthPoolTransferOutDisplay(
                    {
                        payPeriodEnd: m.payPeriodEnd,
                        scheduled: xferScheduled,
                        usedCl: Number(clL.usedThisMonth) || 0,
                        usedCcl: Number(cclL.used) || 0,
                        usedEl: Number(elL.usedThisMonth) || 0,
                        clLockedForPool: clLockForPool,
                    },
                    asOfRegisterXfer,
                    policy
                );
                const xferOutLedgerCl = m.slotPoolTransferOut?.cl;
                const xferOutLedgerCcl = m.slotPoolTransferOut?.ccl;
                const xferOutLedgerEl = m.slotPoolTransferOut?.el;
                const xferOut = {
                    cl: xferOutLedgerCl != null ? xferOutLedgerCl : xferOutComputed.cl,
                    ccl: xferOutLedgerCcl != null ? xferOutLedgerCcl : xferOutComputed.ccl,
                    el: xferOutLedgerEl != null ? xferOutLedgerEl : xferOutComputed.el,
                };

                const clUsedN = Number(clL.usedThisMonth) || 0;
                const cclUsedN = Number(cclL.used) || 0;
                const elUsedN = Number(elL.usedThisMonth) || 0;
                const clPoolBalance = registerPoolBalanceDisplay(
                    policyScheduledCl,
                    clTransferIn,
                    clUsedN,
                    clLockForPool,
                ...
                const cclPoolBalance = registerPoolBalanceDisplay(
                    policyScheduledCco,
                    cclTransferIn,
                    cclUsedN,
                    cclLockForPool,
                ...
                const elPoolBalance = registerPoolBalanceDisplay(
                    policyScheduledEl,
                    elTransferIn,
                    elUsedN,
                    elLockForPool,
                ...
                return {
                    ...
                    cl: {
                        ...
                        /** Pool remainder for the period (matches expanded grid “Bal”); persisted on FY slot via register display sync. */
                        poolBalance: clPoolBalance,
                    ...
                    el: {
                        credited: Math.max(
                            0,
                            (Number(elL.accruedThisMonth) || 0) -
                                (Number(elL.reversalCreditThisMonth) || 0) -
                                (Number(elL.poolTransferInRaw) || 0)
                        ),
    ...
    groupByEmployeeYearly(transactions, fyStart = null, monthsInOrderOverride = null, balanceCarryCapPayrollCycle = null) {
        ...
        transactions.forEach(transaction => {
             ...
             * Pool IN credits still add to accrued/earned for balance chaining; per-type `poolTransferInRaw` tracks
             * carry-in so register "Cr" shows native/policy credits only (same idea as CCL for CL and EL).
             */
            const isMonthlyPoolTransferOut =
                type === 'DEBIT' && autoGen.startsWith('MONTHLY_POOL_TRANSFER_OUT_');
            const isMonthlyPoolTransferIn =
                type === 'CREDIT' && autoGen.startsWith('MONTHLY_POOL_TRANSFER_IN_');
            ...
            if (leaveType === 'CL') {
                ...
                if (type === 'CREDIT') {
                    ...
                    if (isMonthlyPoolTransferIn) {
                        cl.poolTransferInRaw = (Number(cl.poolTransferInRaw) || 0) + days;
                    }
                ...
                if (type === 'CREDIT') {
                    ...
                    if (isMonthlyPoolTransferIn) {
                        el.poolTransferInRaw = (Number(el.poolTransferInRaw) || 0) + days;
                    }
                ...
                if (type === 'CREDIT') {
                    ccl.earned += days;
                    if (isMonthlyPoolTransferIn) {
                        ccl.poolTransferInRaw = (Number(ccl.poolTransferInRaw) || 0) + days;
                    }
        ...
        return Object.values(grouped).map(empEntry => {
            ...
            if (monthsInOrder.length > 0) {
                ...
                monthsInOrder.forEach(m => {
                    yearlySummary.CL.totalCredits += Math.max(
                        0,
                        m.casualLeave.accruedThisMonth +
                            m.casualLeave.earnedCCL -
                            (Number(m.casualLeave.poolTransferInRaw) || 0)
                    );
                    yearlySummary.CL.totalDebits += m.casualLeave.usedThisMonth;
                    
                    yearlySummary.EL.totalCredits += Math.max(
                        0,
                        m.earnedLeave.accruedThisMonth - (Number(m.earnedLeave.poolTransferInRaw) || 0)
                    );
                    yearlySummary.EL.totalDebits += m.earnedLeave.usedThisMonth;
                    
                    yearlySummary.CCL.totalCredits += Math.max(
                        0,
                        m.compensatoryOff.earned - (Number(m.compensatoryOff.poolTransferInRaw) || 0)
                    );
