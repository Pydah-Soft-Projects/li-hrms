...
async function reconcileMonthlyTransfersForLeaveChange(leaveRecord) {
    try {
        const monthlyTransferReconciliationService = require('./monthlyTransferReconciliationService');
        return await monthlyTransferReconciliationService.reconcileForLeaveChange(leaveRecord, { apply: true });
    } catch (error) {
        console.error('[LeaveRegister] Monthly transfer reconciliation failed:', {
