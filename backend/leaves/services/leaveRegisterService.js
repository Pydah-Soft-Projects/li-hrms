...
const { buildCreditsByTypeForSlot } = require('../utils/leaveRegisterScheduledCredits');
...
function pickCreditFromTypeMap(creditsByType, code) {
    if (!creditsByType || typeof creditsByType !== 'object') return 0;
    const u = String(code).toUpperCase();
    const v = creditsByType[u] != null ? creditsByType[u] : creditsByType[code];
    return Math.max(0, Number(v) || 0);
}

function typeUsedNetFromMonthTransactions(transactions, leaveType) {
    const lt = String(leaveType || '').toUpperCase();
    let usedRaw = 0;
    let reversal = 0;
    for (const tx of transactions || []) {
        if (String(tx.leaveType || '').toUpperCase() !== lt) continue;
        const typ = String(tx.transactionType || '').toUpperCase();
        const isTransfer = typ === 'DEBIT' && String(tx.autoGeneratedType || '').startsWith('MONTHLY_POOL_TRANSFER_OUT_');
        const isReversal =
            typ === 'CREDIT' && String(tx.reason || '').includes('Leave Application Cancelled/Reversed');
        if (typ === 'DEBIT' && !isTransfer) usedRaw += Math.max(0, Number(tx.days) || 0);
        if (isReversal) reversal += Math.max(0, Number(tx.days) || 0);
    }
    return Math.max(0, usedRaw - reversal);
}

function typeTransferFromMonthTransactions(transactions, leaveType) {
    const lt = String(leaveType || '').toUpperCase();
    let sum = 0;
    for (const tx of transactions || []) {
        if (String(tx.leaveType || '').toUpperCase() !== lt) continue;
        const typ = String(tx.transactionType || '').toUpperCase();
        const isTransfer = typ === 'DEBIT' && String(tx.autoGeneratedType || '').startsWith('MONTHLY_POOL_TRANSFER_OUT_');
        if (isTransfer) sum += Math.max(0, Number(tx.days) || 0);
    }
    return sum;
}

function typeClosingBalanceFromMonthTransactions(transactions, leaveType) {
    const lt = String(leaveType || '').toUpperCase();
    let last = null;
    for (const tx of transactions || []) {
        if (String(tx.leaveType || '').toUpperCase() !== lt) continue;
        if (tx.closingBalance != null && Number.isFinite(Number(tx.closingBalance))) {
            last = Number(tx.closingBalance);
        }
    }
    return last;
}

function typeTxCountForMonth(transactions, leaveType) {
    const lt = String(leaveType || '').toUpperCase();
    return (transactions || []).filter((tx) => String(tx.leaveType || '').toUpperCase() === lt).length;
}
...
class LeaveRegisterService {
    ...
    async getLeaveRegister(filters = {}, month = null, year = null, options = {}) {
        try {
            ...
            const emptyCapBucket = () => ({
                ...
                pendingLockedCCL: 0,
                pendingLockedEL: 0,
                /** Native approved / in-flight per leave-type code (register “Lk” / per-type cap rows). */
                approvedAppDaysByType: Object.create(null),
                lockedAppDaysByType: Object.create(null),
            });
            let capPolicy = {};
            try {
                capPolicy = await getLeavePolicyResolved();
            } catch {
                capPolicy = {};
            }
            ...
            for (const entry of entriesForHeavyWork) {
                ...
                for (const sub of entry.monthlySubLedgers || []) {
                    ...
                    sub.approvedAppDaysByType = bucket.approvedAppDaysByType || {};
                    sub.lockedAppDaysByType = bucket.lockedAppDaysByType || {};
    ...
    async hydrateRegisterYearView(groupedData, financialYear, options = {}) {
        ...
        let registerLeaveTypeColumns = [];
        try {
            const { getActiveLeaveTypeList } = require('../../settings/services/leavePolicyTypeConfigService');
            registerLeaveTypeColumns = await getActiveLeaveTypeList();
        } catch {
            registerLeaveTypeColumns = [];
        }
        let policy = {};
        try {
        ...
        const ledgerSlice = (sub) => ({
            ...
            approvedAppDaysByType: sub?.approvedAppDaysByType,
            lockedAppDaysByType: sub?.lockedAppDaysByType,
        });
        ...
        for (const entry of groupedData) {
                ...
                for (let orderedIdx = 0; orderedIdx < slotsOrdered.length; orderedIdx++) {
                    ...
                    monthsOut.push({
                        ...
                        scheduled: {
                            ...
                            /** All types (incl. CL/EL/CCL) for dynamic register columns */
                            creditsByType: buildCreditsByTypeForSlot(slot),
                        },
                        ledger: sub
                            ? ledgerSlice(sub)
                            : {
            ...
            entry.registerMonths = monthsOut.map((m) => {
                ...
                const txList = Array.isArray(m.transactions) ? m.transactions : [];
                const creditsMap =
                    m.scheduled?.creditsByType && typeof m.scheduled.creditsByType === 'object'
                        ? m.scheduled.creditsByType
                        : {};
                const laMap =
                    sub?.lockedAppDaysByType && typeof sub.lockedAppDaysByType === 'object'
                        ? sub.lockedAppDaysByType
                        : {};
                const aaMap =
                    sub?.approvedAppDaysByType && typeof sub.approvedAppDaysByType === 'object'
                        ? sub.approvedAppDaysByType
                        : {};
                const byLeaveType = {};
                for (const col of registerLeaveTypeColumns) {
                    const code = String(col?.code || '').toUpperCase();
                    if (!code) continue;
                    if (code === 'CL') {
                        byLeaveType[code] = {
                            code,
                            name: col.name,
                            credited: clRow.credited,
                            used: clRow.used,
                            locked: clRow.locked,
                            transfer: clRow.transfer,
                            typeApplyCap: clRow.typeApplyCap,
                            typeApplyConsumed: clRow.typeApplyConsumed,
                            typeApplyRemaining: clRow.typeApplyRemaining,
                            monthNetBalance: (Number(clRow.credited) || 0) - (Number(clRow.used) || 0),
                            closingBalance: m.ledger?.casualLeave?.balance ?? null,
                            transactionCount: typeTxCountForMonth(txList, code),
                        };
                        continue;
                    }
                    if (code === 'CCL') {
                        byLeaveType[code] = {
                            code,
                            name: col.name,
                            credited: cclRow.credited,
                            used: cclRow.used,
                            locked: cclRow.locked,
                            transfer: cclRow.transfer,
                            typeApplyCap: cclRow.typeApplyCap,
                            typeApplyConsumed: cclRow.typeApplyConsumed,
                            typeApplyRemaining: cclRow.typeApplyRemaining,
                            monthNetBalance: (Number(cclRow.credited) || 0) - (Number(cclRow.used) || 0),
                            closingBalance: m.ledger?.compensatoryOff?.balance ?? null,
                            transactionCount: typeTxCountForMonth(txList, code),
                        };
                        continue;
                    }
                    if (code === 'EL') {
                        byLeaveType[code] = {
                            code,
                            name: col.name,
                            credited: elRow.credited,
                            used: elRow.used,
                            locked: elRow.locked,
                            transfer: elRow.transfer,
                            typeApplyCap: elRow.typeApplyCap,
                            typeApplyConsumed: elRow.typeApplyConsumed,
                            typeApplyRemaining: elRow.typeApplyRemaining,
                            monthNetBalance: (Number(elRow.credited) || 0) - (Number(elRow.used) || 0),
                            closingBalance: m.ledger?.earnedLeave?.balance ?? null,
                            transactionCount: typeTxCountForMonth(txList, code),
                        };
                        continue;
                    }
                    const cred = pickCreditFromTypeMap(creditsMap, code);
                    const usedOth = typeUsedNetFromMonthTransactions(txList, code);
                    const lockedOth = Number(laMap[code] || 0) || 0;
                    const transferOth = typeTransferFromMonthTransactions(txList, code);
                    const tCapOth = getConfiguredMonthlyTypeCap(policy, code);
                    const tOnOth = isPerTypeMonthlyCapActive(policy, code);
                    const nativeOth = (Number(aaMap[code] || 0) || 0) + (Number(laMap[code] || 0) || 0);
                    byLeaveType[code] = {
                        code,
                        name: col.name,
                        credited: cred,
                        used: usedOth,
                        locked: lockedOth,
                        transfer: transferOth,
                        typeApplyCap: tOnOth ? tCapOth : null,
                        typeApplyConsumed: tOnOth ? nativeOth : null,
                        typeApplyRemaining: tOnOth ? Math.max(0, tCapOth - nativeOth) : null,
                        monthNetBalance: cred - usedOth,
                        closingBalance: typeClosingBalanceFromMonthTransactions(txList, code),
                        transactionCount: typeTxCountForMonth(txList, code),
                    };
                }
                ...
                return {
                    ...
                    ccl: cclRow,
                    el: elRow,
                    /** One row per active policy type: same semantics as cl/ccl/el; extras from slot credits + month transactions + per-type cap buckets. */
                    byLeaveType,
                    /** Policy scheduled days per leave type (register pool / grid). */
                    scheduledCreditsByType: m.scheduled?.creditsByType || null,
                };
            });
            ...
            entry.registerLeaveTypeColumns = registerLeaveTypeColumns;
        }
