...
/** Same numeric basis as register UI Bal: policy Cr + carry-in − (used + pending lock) − transfer out. */
function registerPoolBalanceDisplay(policyCr, transferIn, used, locked, transferOut) {
    ...
    const tout = transferOut != null && Number.isFinite(Number(transferOut)) ? Number(transferOut) : null;
    ...
    const ul = u + l;
    if (crN === null && tin === null && tout == null && used == null && locked == null) return null;
    return round2((crN ?? 0) + (tin ?? 0) - ul - (tout ?? 0));
}

/**
 * Sum CL/CCL/EL credits posted as monthly pool carry-in on the FY slot (authoritative vs poolCarryForwardIn).
 * Returns null when no matching MONTHLY_POOL_TRANSFER_IN_* rows exist on that slot.
 */
 ...
 * Returns null when no matching MONTHLY_POOL_TRANSFER_OUT_* rows exist on that slot.
 */
    ...
    async hydrateRegisterYearView(groupedData, financialYear, options = {}) {
        ...
        for (const entry of groupedData) {
            ...
            entry.registerMonths = monthsOut.map((m) => {
                ...
                const clLockForPool = sub != null ? clLockedDisplay : policyLock;
