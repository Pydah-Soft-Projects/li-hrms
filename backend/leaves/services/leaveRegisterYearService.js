...
function allocateLockedByPriorityForSlot(slot, scheduled, used, elInPool) {
  let remaining = round2(Math.max(Number(slot?.lockedCredits) || 0, Number(slot?.monthlyApplyLocked) || 0));
  const locked = { cl: 0, ccl: 0, el: 0 };
  const clAvailable = round2(Math.max(0, (Number(scheduled.cl) || 0) - (Number(used.cl) || 0)));
  locked.cl = round2(Math.min(remaining, clAvailable));
  remaining = round2(Math.max(0, remaining - locked.cl));
  const cclAvailable = round2(Math.max(0, (Number(scheduled.ccl) || 0) - (Number(used.ccl) || 0)));
  locked.ccl = round2(Math.min(remaining, cclAvailable));
  remaining = round2(Math.max(0, remaining - locked.ccl));
  if (elInPool) {
    const elAvailable = round2(Math.max(0, (Number(scheduled.el) || 0) - (Number(used.el) || 0)));
    locked.el = round2(Math.min(remaining, elAvailable));
  }
  return locked;
}
...
function countQualifyingMonthlyPoolCarryEdges(months, asOf, policy, options = {}) {
  ...
  for (let p = 0; p < ordered.length - 1; p++) {
    ...
    const usedCcl = sumUsedDaysForType(cur, 'CCL');
    const usedEl = sumUsedDaysForType(cur, 'EL');
    const locked = allocateLockedByPriorityForSlot(
      cur,
      { cl: cur.clCredits, ccl: cur.compensatoryOffs, el: cur.elCredits },
      { cl: usedCl, ccl: usedCcl, el: usedEl },
      elRoll
    );
    let clCarry = Math.max(0, round2(Number(cur.clCredits) - usedCl - locked.cl));
    if (!clRoll) clCarry = 0;
    let cclCarry = Math.max(0, round2(Number(cur.compensatoryOffs) - usedCcl - locked.ccl));
    if (!cclRoll) cclCarry = 0;
    let elCarry = Math.max(0, round2(Number(cur.elCredits) - usedEl - locked.el));
    if (!elRoll) elCarry = 0;
