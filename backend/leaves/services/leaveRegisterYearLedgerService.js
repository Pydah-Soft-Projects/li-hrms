...
/**
 * Total days posted as auto EL accrual (CREDIT + EARNED_LEAVE) for a payroll cycle month.
 */
async function getEarnedLeaveAutoCreditTotalForPayrollCycle(employeeId, payrollMonth, payrollYear) {
  const cycleTargetDate = new Date(Number(payrollYear), Number(payrollMonth) - 1, 15);
  const fyWrap = await dateCycleService.getFinancialYearForDate(cycleTargetDate);
  const fyName = fyWrap.name;
  const doc = await LeaveRegisterYear.findOne({ employeeId, financialYear: fyName });
  if (!doc) return { total: 0, count: 0, financialYear: fyName };
  const mi = findMonthIndex(doc, payrollMonth, payrollYear);
  if (mi < 0) return { total: 0, count: 0, financialYear: fyName };
  let total = 0;
  let count = 0;
  for (const t of doc.months[mi].transactions || []) {
    if (
      t.leaveType === 'EL' &&
      String(t.transactionType || '').toUpperCase() === 'CREDIT' &&
      t.autoGeneratedType === 'EARNED_LEAVE'
    ) {
      total += roundHalf(Number(t.days) || 0);
      count += 1;
    }
  }
  return { total: roundHalf(total), count, financialYear: fyName };
}

/**
 * Remove all auto EL accrual credits for that payroll month, rebuild EL running balances, sync monthly-apply fields.
 * Does not remove manual EL credits or payroll-use debits.
 */
async function removeEarnedLeaveAutoCreditsForPayrollCycle(employeeId, payrollMonth, payrollYear) {
  const cycleTargetDate = new Date(Number(payrollYear), Number(payrollMonth) - 1, 15);
  const fyWrap = await dateCycleService.getFinancialYearForDate(cycleTargetDate);
  const doc = await LeaveRegisterYear.findOne({ employeeId, financialYear: fyWrap.name });
  if (!doc) return { removedCount: 0, previousTotal: 0 };
  const mi = findMonthIndex(doc, payrollMonth, payrollYear);
  if (mi < 0) return { removedCount: 0, previousTotal: 0 };
  ...
  for (let i = 0; i < txs.length; i++) {
    ...
    if (
      ...
      previousTotal += roundHalf(Number(t.days) || 0);
    }
  }
  if (!spliceIndices.length) return { removedCount: 0, previousTotal: 0 };
  for (let j = spliceIndices.length - 1; j >= 0; j--) {
    txs.splice(spliceIndices[j], 1);
  }
  doc.markModified(`months.${mi}.transactions`);
  doc.markModified(`months.${mi}`);
  await doc.save();
  await recalculateRegisterBalances(employeeId, 'EL', null);
  const syncDate = slot.payPeriodStart || cycleTargetDate;
  await leaveRegisterYearMonthlyApplyService.syncStoredMonthApplyFieldsForEmployeeDate(employeeId, syncDate);
  return { removedCount: spliceIndices.length, previousTotal: roundHalf(previousTotal) };
}
...
module.exports = {
  ...
  getEarnedLeaveAutoCreditTotalForPayrollCycle,
  removeEarnedLeaveAutoCreditsForPayrollCycle,
