...
async function addTransaction(transactionData) {
  ...
  if (immediateMonthlyApplySync) {
    ...
    scheduleRegisterDisplaySnapshotSync(transactionData.employeeId, doc.financialYear);
  } else {
  ...
  const autoType = String(transactionData.autoGeneratedType || '');
  const transferAuto = autoType.startsWith('MONTHLY_POOL_TRANSFER_OUT_') || autoType.startsWith('MONTHLY_POOL_TRANSFER_IN_');
  if (!transferAuto) {
    leaveRegisterYearMonthlyApplyService.scheduleReconcileMonthCarryForEmployeeDate(
      transactionData.employeeId,
      transactionData.startDate
    );
  }
...
async function findYearDocsForRegisterFilters(filters, fyName) {
  ...
  if (filters.divisionId || filters.departmentId || filters.designationId || filters.employee_group_id) {
    // Scope filtering should not hide historical register rows for inactive employees.
    // If they have FY docs/transactions and match the requested org scope, keep them visible.
    const empQ = {};
