...
async function loadHolidayActor(req) {
    ...
    const user = await User.findById(userId)
        .select('name email role roles featureControl managedHolidayGroupIds customRoles')
...
/**
 * Scoped managers: "global" create applies to all assigned groups (bulk GROUP rows), not org-wide GLOBAL masters.
 */
function normalizeHolidayWritePayload(actor, body) {
    ...
    if (canManageGlobal(actor)) return out;

    const managed = getManagedGroupIdStrings(actor);
    if (managed.length === 0) {
        const err = new Error('No holiday groups assigned to your account');
        err.statusCode = 403;
        throw err;
    }

    if (out.scope === 'GLOBAL' || out.isMaster) {
        let targetIds = managed;
